...
def _ssim_compute(
    ...
    input_list = torch.cat((preds, target, preds * preds, target * target, preds * target))  # (5 * B, C, H, W) x, y, x^2, y^2, xy
    outputs = F.conv2d(input_list, kernel, groups=channel)
    output_list = [outputs[x * preds.size(0) : (x + 1) * preds.size(0)] for x in range(len(outputs))]
    # output_list window*x, window*y, window*x^2, window*y^2, window*xy
    mu_pred_sq = output_list[0].pow(2) # mu1_sq
    mu_target_sq = output_list[1].pow(2) # mu2_sq
    mu_pred_target = output_list[0] * output_list[1] # mu1_mu2

    sigma_pred_sq = output_list[2] - mu_pred_sq  # sigma1_sq
    sigma_target_sq = output_list[3] - mu_target_sq # sigma2_sq
    sigma_pred_target = output_list[4] - mu_pred_target # sigma12

    upper = 2 * sigma_pred_target + c2 # v1
    lower = sigma_pred_sq + sigma_target_sq + c2 # v2
    

    ssim_idx = ((2 * mu_pred_target + c1) * upper) / ((mu_pred_sq + mu_target_sq + c1) * lower)
    ssim_idx = ssim_idx[..., pad_h:-pad_h, pad_w:-pad_w]
    ssim_score = reduce(ssim_idx, reduction)

    if full:
        contrast_sensitivity = (upper / lower).mean()
        # contrast_sensitivity = contrast_sensitivity.mean()
...
_MS_SSIM_WEIGHTS = FloatTensor([0.0448, 0.2856, 0.3001, 0.2363, 0.1333])

def ms_ssim(
    preds: Tensor,
    target: Tensor,
    max_val: Optional[float] = None,
    power_factors: Tuple[float] =_MS_SSIM_WEIGHTS,
    filter_size: int = 11,
    ...
    """
    Computes the MS-SSIM between preds and target. 
    """

    preds, target = _ms_ssim_update(preds, target)
    return _ms_ssim_compute(preds, target, max_val, filter_size, filter_sigma, k1, k2)
...
def _ms_ssim_compute(
    ...
    if max_val is None:
        max_val = max(preds.max() - preds.min(), target.max() - target.min())

    device = preds.device
    power_factors = power_factors.to(device)
    levels = weights.size(0)
    ssims = []
    mean_con_sens = []
    for _ in range(levels):
        similarity, contrast_sensitivity = 
